import { describe, it, assert, DA, JSDOM, HTML5 } from "./helper.mjs";
describe("DA.HTML#title", function () {
    it("updates the title", function () {
        const dom = new JSDOM(HTML5);
        let h = new DA.HTML(dom.window);
        h.title("new title");
        assert.equal(dom.window.document.getElementsByTagName('title')[0].innerHTML, "new title");
    });
});
describe("DA.HTML#link", function () {
...
describe("DA.HTML#meta", function () {
    it("creates a META element in HEAD", function () {
        const dom = new JSDOM(HTML5);
        let h = new DA.HTML(dom.window);
        h.meta({ content: "/style.css", "http-equiv": "default-style" });
        assert.equal(dom.window.document.querySelectorAll('meta')[1].outerHTML, `<meta content="/style.css" http-equiv="default-style">`);
    });
    it("updates meta charset", function () {
        const dom = new JSDOM(HTML5);
        let h = new DA.HTML(dom.window);
        h.meta({ charset: "utf-16" });
        assert.equal(dom.window.document.querySelector('meta').outerHTML, `<meta charset="utf-16">`);
    });
});
describe("DA.HTML#body", function () {
    it("updates the attributes of the BODY element", function () {
        const dom = new JSDOM();
        let h = new DA.HTML(dom.window);
        h.body(".ready-now", function () {
        });
        assert.equal(dom.window.document.querySelector('body').getAttribute("class"), `ready-now`);
    });
    it("allows child nodes", function () {
        ...
        assert.equal(dom.window.document.querySelector("script").parentNode.tagName, "BODY");
    });
});
